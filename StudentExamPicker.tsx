...
export const StudentExamPicker: React.FC<Props> = ({ onBack }) => {
  ...
  // Deduplicate and normalize class names against official school roster
  const classNames = useMemo(() => {
    const publishedNormSet = new Set(
      allOptions.map((o) => (o.className || '').trim().toLowerCase())
    );
    // Prefer matching official classes in canonical order
    const matchingOfficial = CLASS_NAMES.filter((c) =>
      publishedNormSet.has(c.toLowerCase())
    );
    if (matchingOfficial.length > 0) {
      return matchingOfficial;
    }
    return CLASS_NAMES;
  }, [allOptions]);
  ...
  const subjectsForClass = useMemo(() => {
    ...
    const matched = allOptions.filter(
      (o) => (o.className || '').trim().toLowerCase() === targetClass
    );
    return Array.from(new Set(matched.map((o) => o.subject.trim()))).sort();
  ...
  const examsForSubject = useMemo(() => {
    ...
    const matched = allOptions.filter(
      (o) =>
        (o.className || '').trim().toLowerCase() === targetClass &&
        (o.subject || '').trim().toLowerCase() === targetSub
    );
    // Deduplicate by examId
    const seen = new Set<string>();
    return matched.filter((o) => {
      if (seen.has(o.examId)) return false;
      seen.add(o.examId);
      return true;
    });
      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-8 space-y-5">
        ...
        {stage === 'instructions' && exam && (
          <form onSubmit={handleStartWithToken} className="space-y-4">
            ...
            <div className="text-xs text-stone-600 bg-stone-50 border border-stone-200 rounded-xl p-3.5 space-y-1.5">
              <p>&bull; Jawab tiap soal dengan teliti. Setiap pilihan jawabanmu menentukan takdir langkah berikutnya.</p>
              <p>&bull; Kalau jawaban keliru, kamu akan tersesat ke soal jalur cabang pengembaraan sebelum bisa menemukan jalan kembali.</p>
              <p>&bull; Kerjakan dengan jujur. Jangan keluar dari layar penuh atau berpindah tab selama ujian berlangsung.</p>
            </div>
            <div>
