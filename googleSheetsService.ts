...
export const testGoogleSheetsWebhook = async (
  webhookUrl: string
): Promise<{ successCount: number; errorCount: number; message: string; success: boolean }> => {
  if (!webhookUrl.trim() || !webhookUrl.startsWith('http')) {
    return { success: false, successCount: 0, errorCount: 1, message: 'URL Webhook tidak valid. Pastikan diawali dengan https://script.google.com/...' };
  }
